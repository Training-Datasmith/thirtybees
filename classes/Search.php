...
class Search_Core
{
     ...
     */
    public static function find($id_lang, $expr, $page_number = 1, $page_size = 1, $order_by = 'position', ?string $order_way = 'desc', $ajax = false, $use_cookie = true, ?Context $context = null)
    {
        if (!$context) {
            ...
            if (!empty($word) && strlen($word) >= (int) Configuration::get('PS_SEARCH_MINWORDLEN')) {
                ...
                $intersect_array[] = 'SELECT DISTINCT si.id_product
					...
					WHERE sw.id_lang = ' . (int) $id_lang . '
						...
						AND sw.word LIKE
					' . ($word[0] == '-' ? ' \'' . $start_search . p_sql(mb_substr($word, 1, PS_SEARCH_MAX_WORD_LENGTH)) . $end_search . '\'' : ' \'' . $start_search . p_sql(mb_substr($word, 0, PS_SEARCH_MAX_WORD_LENGTH)) . $end_search . '\'');
                if ($word[0] != '-') {
                    $score_array[] = 'sw.word LIKE \'' . $start_search . p_sql(mb_substr($word, 0, PS_SEARCH_MAX_WORD_LENGTH)) . $end_search . '\'';
                }
            } else {
                unset($words[$key]);
            }
        ...
        $search_sql = (new Db_Query())->select('DISTINCT `cp`.`id_product`')->from('category_product', 'cp')->inner_join_multishop('category', 'c', 'cs', '`cp`.`id_category` = `c`.`id_category`')->inner_join_multishop('product', 'p', 'ps', '`cp`.`id_product` = `p`.`id_product`')->where('`cs`.`active` = 1')->where('`ps`.`active` = 1')->where('`ps`.`visibility` IN ("both", "search")')->where('`ps`.`indexed` = 1');
        if (Group::is_feature_active()) {
            ...
            $search_sql->inner_join('category_group', 'cg', $on);
        }
        if ((int) Configuration::get('PS_SEARCH_INSTOCK')) {
            $on = '`cp`.`id_product` = `sa`.`id_product` ' . Stock_Available::add_sql_shop_restriction(null, null, 'sa');
            ...
            $search_sql->inner_join('stock_available', 'sa', $on);
        }
        $results = $db->get_array($search_sql);
        $eligible_products = [];
        ...
        foreach ($intersect_array as $query) {
            foreach ($db->get_array($query) as $row) {
        ...
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity,
			 ...
			 image_shop.`id_image` id_image, il.`legend`, m.`name` manufacturer_name ' . $score . ',
				...
				) > 0 new' . (Combination::is_feature_active() ? ', product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity, IFNULL(product_attribute_shop.`id_product_attribute`,0) id_product_attribute' : '') . '
				FROM ' . _DB_PREFIX_ . 'product p
				' . Shop::add_sql_association('product', 'p') . '
				INNER JOIN `' . _DB_PREFIX_ . 'product_lang` pl ON (
					p.`id_product` = pl.`id_product`
				...
				' . (Combination::is_feature_active() ? 'LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_shop` product_attribute_shop
        ...
        if (!$result) {
            ...
            $result_properties = Product::get_products_properties((int) $id_lang, $result);
        }
    ...
    public static function sanitize($string, $id_lang, $indexation = false, $iso_code = false)
    {
        ...
        if ($indexation) {
            ...
            foreach ($words as $word) {
                ...
                if (Validate::is_loaded_object($alias)) {
        ...
        if (in_array($iso_code, ['zh', 'tw', 'ja']) && function_exists('mb_strlen')) {
            ...
            foreach (explode(' ', (string) $string) as $mb_word) {
                if (strlen(Tools::replace_accented_chars($mb_word)) == mb_strlen(Tools::replace_accented_chars($mb_word))) {
                    $letters .= $mb_word . ' ';
                } else {
                    $symbols .= $mb_word . ' ';
                }
            ...
            $min_word_len = (int) Configuration::get('PS_SEARCH_MINWORDLEN');
            if ($min_word_len > 1) {
                $min_word_len -= 1;
                $string = preg_replace('/(?<=\s)[^\s]{1,' . $min_word_len . '}(?=\s)/Su', ' ', (string) $string);
                $string = preg_replace('/^[^\s]{1,' . $min_word_len . '}(?=\s)/Su', '', (string) $string);
                $string = preg_replace('/(?<=\s)[^\s]{1,' . $min_word_len . '}$/Su', '', (string) $string);
                $string = preg_replace('/^[^\s]{1,' . $min_word_len . '}$/Su', '', (string) $string);
            }
        }
        return Tools::replace_accented_chars(trim((string) preg_replace('/\s+/', ' ', (string) $string)));
    }
    /**
     * @param bool $full
     ...
     * @throws PrestaShopException
     */
    public static function indexation($full = false, $specific_product_id = false): bool
    {
        $db = Db::get_instance();
        $specific_product_id = (int) $specific_product_id;
        if ($specific_product_id) {
            $full = false;
        }
        if ($full && Context::get_context()->shop->get_context() == Shop::CONTEXT_SHOP) {
            ...
            $db->execute('DELETE si FROM `' . _DB_PREFIX_ . 'search_index` si
				INNER JOIN `' . _DB_PREFIX_ . 'product` p ON (p.id_product = si.id_product)
				' . Shop::add_sql_association('product', 'p', true, null, true) . '
				WHERE product_shop.`visibility` IN ("both", "search")
				AND product_shop.`active` = 1
				AND ' . ($specific_product_id ? 'p.`id_product` = ' . (int) $specific_product_id : 'product_shop.`indexed` = 0'));
            $db->execute('UPDATE `' . _DB_PREFIX_ . 'product` p
				' . Shop::add_sql_association('product', 'p', true, null, true) . '
				SET p.`indexed` = 0, product_shop.`indexed` = 0
				WHERE product_shop.`visibility` IN ("both", "search")
        ...
        $weight_array = ['pname' => (int) Configuration::get('PS_SEARCH_WEIGHT_PNAME'), 'reference' => (int) Configuration::get('PS_SEARCH_WEIGHT_REF'), 'pa_reference' => (int) Configuration::get('PS_SEARCH_WEIGHT_REF'), 'supplier_reference' => (int) Configuration::get('PS_SEARCH_WEIGHT_REF'), 'pa_supplier_reference' => (int) Configuration::get('PS_SEARCH_WEIGHT_REF'), 'ean13' => (int) Configuration::get('PS_SEARCH_WEIGHT_REF'), 'pa_ean13' => (int) Configuration::get('PS_SEARCH_WEIGHT_REF'), 'upc' => (int) Configuration::get('PS_SEARCH_WEIGHT_REF'), 'pa_upc' => (int) Configuration::get('PS_SEARCH_WEIGHT_REF'), 'description_short' => (int) Configuration::get('PS_SEARCH_WEIGHT_SHORTDESC'), 'description' => (int) Configuration::get('PS_SEARCH_WEIGHT_DESC'), 'cname' => (int) Configuration::get('PS_SEARCH_WEIGHT_CNAME'), 'mname' => (int) Configuration::get('PS_SEARCH_WEIGHT_MNAME'), 'tags' => (int) Configuration::get('PS_SEARCH_WEIGHT_TAG'), 'attributes' => (int) Configuration::get('PS_SEARCH_WEIGHT_ATTRIBUTE'), 'features' => (int) Configuration::get('PS_SEARCH_WEIGHT_FEATURE')];
        ...
        $total_languages = count(Language::get_i_ds(false));
        $sql_attribute = Search::get_sql_product_attribute_fields($weight_array);
        // Products are processed 50 by 50 in order to avoid overloading MySQL
        $search_index_batch = [];
        while ($products = Search::get_products_to_index($total_languages, $specific_product_id, 50, $weight_array)) {
            $processed_products = [];
            // Now each non-indexed product is processed one by one, langage by langage
            foreach ($products as $product) {
                $product_id = (int) $product['id_product'];
                $language_id = (int) $product['id_lang'];
                $shop_id = (int) $product['id_shop'];
                if ($weight_array['tags']) {
                    $product['tags'] = Search::get_tags($db, $product_id, $language_id);
                }
                if ($weight_array['attributes']) {
                    $product['attributes'] = Search::get_attributes($db, $product_id, $language_id);
                }
                if ($weight_array['features']) {
                    $product['features'] = Search::get_features($db, $product_id, $language_id);
                }
                if ($sql_attribute) {
                    $attribute_fields = Search::get_attributes_fields($db, $product_id, $sql_attribute);
                    if ($attribute_fields) {
                        $product['attributes_fields'] = $attribute_fields;
                    }
                }
                // Data must be cleaned of html, bad characters, spaces and anything, then if the resulting words are long enough, they're added to the array
                $product_words = static::get_product_words($product, $weight_array);
                // If we find words that need to be indexed, they're added to the word table in the database
                if ($product_words) {
                    $words_ids = static::get_or_create_words(array_keys($product_words), $shop_id, $language_id);
                    foreach ($words_ids as $word => $word_id) {
                        $weight = $product_words[$word];
                        $search_index_batch[] = '(' . $product_id . ',' . $word_id . ',' . $weight . ')';
                        if (++$count_words % 50 === 0) {
                            Search::save_index($search_index_batch);
                            $search_index_batch = [];
                        }
                    }
                }
                $processed_products[$product_id] = $product_id;
            }
            Search::set_products_as_indexed($processed_products);
        }
     ...
     */
    protected static function get_sql_product_attribute_fields($weight_array): string
    {
        $sql = '';
     ...
     * @throws PrestaShopException
     */
    protected static function get_products_to_index($total_languages, $id_product = false, $limit = 50, $weight_array = [])
    {
        $ids = null;
        $conn = Db::read_only();
        if (!$id_product) {
            ...
            $sql = 'SELECT p.id_product FROM ' . _DB_PREFIX_ . 'product p
				...
				LIMIT ' . (int) $limit;
            $ids = array_column($conn->get_array($sql), 'id_product');
        }
     ...
     * @throws PrestaShopException
     */
    public static function get_tags($db, $id_product, $id_lang): string
    {
        $tags = '';
        $tags_array = $db->get_array('
     ...
     * @throws PrestaShopException
     */
    public static function get_attributes($db, $id_product, $id_lang): string
    {
        if (!Combination::is_feature_active()) {
            return '';
        }
        $attributes = '';
        $attributes_array = $db->get_array('
     ...
     * @throws PrestaShopException
     */
    public static function get_features($db, $id_product, $id_lang): string
    {
        if (!Feature::is_feature_active()) {
            return '';
        }
        $features = '';
        $features_array = $db->get_array('
     ...
     * @throws PrestaShopException
     */
    protected static function get_attributes_fields($db, $id_product, string $sql_attribute)
    {
        return $db->get_array('SELECT id_product ' . $sql_attribute . ' FROM ' . _DB_PREFIX_ . 'product_attribute pa WHERE pa.id_product = ' . (int) $id_product);
    }
    /**
     * @param array<string, int> $weightArray
     * @param string $value
     ...
     * @throws PrestaShopException
     */
    protected static function fill_product_array(array &$product_array, array $weight_array, string $key, $value, $id_lang, $iso_code)
    {
        $weight = (int) ($weight_array[$key] ?? 0);
        if ($weight) {
     ...
     * @throws PrestaShopException
     */
    protected static function save_index($tuples)
    {
        if ($tuples) {
     ...
     * @throws PrestaShopException
     */
    protected static function set_products_as_indexed($ids)
    {
     ...
     * @throws PrestaShopException
     */
    public static function remove_products_search_index($ids): void
    {
        $ids = array_filter(array_map(intval(...), $ids));
        if ($ids) {
            $conn = Db::get_instance();
     ...
     * @throws PrestaShopException
     */
    public static function search_tag($id_lang, $tag, $count = false, $page_number = 0, $page_size = 10, $order_by = false, $order_way = false, $use_cookie = true, ?Context $context = null)
    {
        if (!$context) {
            $context = Context::get_context();
        }
        if (!is_numeric($page_number) || !is_numeric($page_size) || !Validate::is_bool($count) || !Validate::is_valid_search($tag) || $order_by && !$order_way || $order_by && !Validate::is_order_by($order_by) || $order_way && !Validate::is_order_by($order_way)) {
            return false;
        }
        if ($page_number < 1) {
            $page_number = 1;
        ...
        $conn = Db::read_only();
        if ($count) {
            return (int) $conn->get_value('SELECT COUNT(DISTINCT pt.`id_product`) nb
			FROM
			`' . _DB_PREFIX_ . 'tag` t
			...
			' . (Group::is_feature_active() ? 'LEFT JOIN `' . _DB_PREFIX_ . 'category_group` cg ON (cg.`id_category` = cp.`id_category`)' : '') . '
			WHERE product_shop.`active` = 1
			AND p.visibility IN (\'both\', \'search\')
			AND cs.`id_shop` = ' . (int) Context::get_context()->shop->id . '
			' . $sql_groups . '
			AND t.`name` LIKE \'%' . p_sql($tag) . '%\'');
        }
        $sql = 'SELECT DISTINCT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, pl.`description_short`, pl.`link_rewrite`, pl.`name`, pl.`available_now`, pl.`available_later`,
				...
				LIMIT ' . (int) (($page_number - 1) * $page_size) . ',' . (int) $page_size;
        if (!$result = $conn->get_array($sql)) {
            return false;
        }
        return Product::get_products_properties((int) $id_lang, $result);
    }
    /**
     ...
     * @throws PrestaShopException
     */
    protected static function get_product_words(array $product_data, array $weight_array): array
    {
        $iso_code = $product_data['iso_code'];
        $language_id = (int) $product_data['id_lang'];
        $product_words = [];
        foreach ($product_data as $key => $value) {
            if ($key === 'attributes_fields') {
                foreach ($value as $pa_array) {
                    foreach ($pa_array as $pa_key => $pa_value) {
                        Search::fill_product_array($product_words, $weight_array, $pa_key, $pa_value, $language_id, $iso_code);
                    }
                }
            } else {
                Search::fill_product_array($product_words, $weight_array, $key, $value, $language_id, $iso_code);
            }
        }
        return array_filter($product_words);
    }
    /**
     ...
     * @throws PrestaShopException
     */
    protected static function get_existing_words(array $words, int $shop_id, int $language_id): array
    {
        $result = [];
        if ($words) {
            $imploded = implode(', ', array_map(fn($word) => '\'' . p_sql($word) . '\'', $words));
            $sql = (new Db_Query())->select('sw.id_word, sw.word')->from('search_word', 'sw')->where("sw.word IN ({$imploded})")->where('sw.id_lang = ' . $language_id)->where('sw.id_shop = ' . $shop_id);
     ...
     * @throws PrestaShopException
     */
    private static function get_or_create_words(array $words, int $shop_id, int $language_id): array
    {
        $existing_words = static::get_existing_words($words, $shop_id, $language_id);
        $missing_words = array_diff($words, array_keys($existing_words));
        if ($missing_words) {
            $data = [];
            foreach ($missing_words as $word) {
                $data[] = ['id_shop' => $shop_id, 'id_lang' => $language_id, 'word' => $word];
            }
            Db::get_instance()->insert('search_word', $data, false, true, Db::INSERT_IGNORE);
            $existing_words = static::get_existing_words($words, $shop_id, $language_id);
        }
