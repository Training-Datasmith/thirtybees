...
$layers = Tools::get_value('layers');
$width = Tools::get_value('width');
$height = Tools::get_value('height');
...
$graph = new $module();
$graph->set_employee($id_employee);
$graph->set_lang($id_lang);
if ($option) {
    $graph->set_option($option, $layers);
}
$graph->create_graph(null, $type, $width, $height, $layers);
$graph->draw();