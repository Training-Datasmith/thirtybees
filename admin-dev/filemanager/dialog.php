...
if (Tools::is_submit('submit')) {
    ...
    $subdir = get_sub_dir(Tools::get_value('fldr'));
    ...
    $popup = Tools::get_int_value('popup');
    // resolve view type
    $view = Tools::get_isset('view') ? set_view_type(Tools::get_int_value('view', 0)) : get_view_type();
    $sort_by = Tools::get_isset('sort_by') ? set_sort_by(Tools::get_value('sort_by')) : get_sort_by();
    $descending = Tools::get_isset('descending') ? set_descending(Tools::get_value('descending') === 'true') : get_descending();
    $lang = resolve_language();
    $filter = fix_filename(Tools::get_value('filter', ''));
    $type = Tools::get_int_value('type', 0);
    $get_params = http_build_query(['type' => $type, 'lang' => $lang, 'popup' => $popup ? 1 : 0, 'fldr' => '']);
    ?>
	...
	<head>
		...
		<?php 
    foreach (Media::get_jquery_path() as $j_query_path) {
        ?>
    ...
    echo implode("','", get_file_extensions('image'));
    ?>');
			var allowed_ext = new Array('<?php 
    echo implode("','", get_file_extensions());
    ?>');
    ...
    ?>",
				...
				maxFilesize: <?php 
    echo Tools::get_max_upload_size() / (1024 * 1024);
    ?>, // MB
				url: "upload.php",
    ...
    echo Tools::safe_output(FILE_MANAGER_FILE_NUMBER_LIMIT_JS);
    ?>"/>
	<input type="hidden" id="descending" value="<?php 
    echo $descending ? 'true' : 'false';
    ?>"/>
	<input type="hidden" id="current_url" value="<?php 
    echo str_replace(['&filter=' . $filter], [''], 'http://' . $_SERVER['HTTP_HOST'] . Tools::safe_output($_SERVER['REQUEST_URI']));
    ?>"/>
	<input type="hidden" id="lang_show_url" value="<?php 
    echo Tools::safe_output(lang_Show_url);
    ?>"/>
	<input type="hidden" id="lang_extract" value="<?php 
    echo Tools::safe_output(lang_Extract);
    ?>"/>
    ...
    echo Tools::safe_output(lang_Edit_image);
    ?>"/>

    <div class="uploader">
        <center>
            <button class="btn btn-inverse close-uploader">
                <i class="icon-backward icon-white"></i> <?php 
    echo Tools::safe_output(lang_Return_Files_List);
    ?></button>
        </center>
    ...
    echo Tools::safe_output($subdir);
    ?>"/>
                        <input type="hidden" name="path_thumb" value="<?php 
    echo Tools::safe_output($subdir);
    ?>"/>

                        <div class="fallback">
                            <?php 
    echo lang_Upload_file;
    ?>:<br/>
                            <input name="file" type="file"/>
                            <input type="hidden" name="fldr" value="<?php 
    echo Tools::safe_output($subdir);
    ?>"/>
    ...
    echo Tools::safe_output($lang);
    ?>"/>
                            <input type="hidden" name="filter" value="<?php 
    echo Tools::safe_output($filter);
    ?>"/>
    ...
    echo Tools::safe_output(lang_Upload_base_help);
    ?></div>
            </div>
        </div>
	...
	<div class="navbar navbar-fixed-top">
		<div class="navbar-inner">
			<div class="container-fluid">
				...
				<div class="brand"><?php 
    echo Tools::safe_output(lang_Toolbar);
    ?> -></div>
					...
					<div class="filters">
						<div class="row-fluid">
							<div class="span3 half">
								<span><?php 
    echo Tools::safe_output(lang_Actions);
    ?>:</span>
                                <button class="tip btn upload-btn" title="<?php 
    echo Tools::safe_output(lang_Upload_file);
    ?>">
    ...
    echo Tools::safe_output(lang_View_boxes);
    ?>">
									<i class="icon-th <?php 
    if ($view == 0) {
        echo 'icon-white';
    ...
    echo Tools::safe_output(lang_View_list);
    ?>">
    ...
    echo Tools::safe_output(lang_View_columns_list);
    ?>">
    ...
    ?>"></i>
							...
							<div class="span6 types">
								<span><?php 
    echo Tools::safe_output(lang_Filters);
    ?>:</span>
								<?php 
    if ($type !== 1 && $type !== 3) {
        ?>
        ...
        echo Tools::safe_output(lang_Files);
        ?>" for="select-type-1" class="tip btn ff-label-type-1">
                                        <i class="icon-file"></i>
                                    </label>
									<input id="select-type-2" name="radio-sort" type="radio" data-item="ff-item-type-2" class="hide"/>
									<label id="ff-item-type-2" title="<?php 
        echo Tools::safe_output(lang_Images);
        ?>" for="select-type-2" class="tip btn ff-label-type-2">
                                        <i class="icon-picture"></i>
                                    </label>
									<input id="select-type-3" name="radio-sort" type="radio" data-item="ff-item-type-3" class="hide"/>
									<label id="ff-item-type-3" title="<?php 
        echo Tools::safe_output(lang_Archives);
        ?>" for="select-type-3" class="tip btn ff-label-type-3">
        ...
        echo Tools::safe_output(lang_Videos);
        ?>" for="select-type-4" class="tip btn ff-label-type-4">
                                        <i class="icon-film"></i>
                                    </label>
									<input id="select-type-5" name="radio-sort" type="radio" data-item="ff-item-type-5" class="hide"/>
									<label id="ff-item-type-5" title="<?php 
        echo Tools::safe_output(lang_Music);
        ?>" for="select-type-5" class="tip btn ff-label-type-5">
                                        <i class="icon-music"></i>
                                    </label>
								<?php 
    }
    ?>
								<input accesskey="f" type="text" class="filter-input" id="filter-input" name="filter" placeholder="<?php 
    echo fix_strtolower(lang_Text_filter);
    ?>..." value="<?php 
    echo Tools::safe_output($filter);
    ?>"/>
                                <?php 
    if ($n_files > FILE_MANAGER_FILE_NUMBER_LIMIT_JS) {
        ?>
    ...
    echo Tools::safe_output(lang_All);
    ?>" <?php 
    if (Tools::get_value('type') == 1 || Tools::get_value('type') == 3) {
        ?>style="visibility: hidden;" <?php 
    }
    ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all">
    ...
    if (!empty($bc)) {
        foreach ($bc as $k => $b) {
            ...
            if ($k == count($bc) - 2) {
                ?>
						<li class="active"><?php 
                echo Tools::safe_output($b);
                ?></li><?php 
            } elseif ($b != '') {
                ?>
						<li><a href="<?php 
                echo Tools::safe_output($link . $tmp_path);
                ?>"><?php 
                echo Tools::safe_output($b);
                ?></a></li>
    ...
    ?>
			...
			<li class="pull-right">
				<a id="refresh" class="btn-small" href="dialog.php?<?php 
    echo Tools::safe_output($get_params . $subdir . '&' . uniqid());
			...
			<li class="pull-right">
				<div class="btn-group">
					...
					<ul class="dropdown-menu pull-left sorting">
						<li>
							<center><strong><?php 
    echo Tools::safe_output(lang_Sorting);
    ?></strong></center>
						</li>
    ...
    echo Tools::safe_output(lang_Filename);
    ?></a></li>
    ...
    echo Tools::safe_output(lang_Date);
    ?></a></li>
    ...
    echo Tools::safe_output(lang_Size);
    ?></a></li>
    ...
    echo Tools::safe_output(lang_Type);
    ?></a></li>
					</ul>
				</div>
			</li>
	...
	<div class="span12">
	<?php 
    if (@opendir(FILE_MANAGER_BASE_DIR . $subdir) === false) {
        ?>
		<br/>
	...
	<?php 
        ...
        ?>
	<h4 id="help"><?php 
        echo Tools::safe_output(lang_Swipe_help);
        ?></h4>
	<?php 
        if (isset($folder_message)) {
            ?>
		<div class="alert alert-block"><?php 
            echo Tools::safe_output($folder_message);
            ?></div>
	<?php 
        }
        ?>
    ...
    <div class="sorter-container <?php 
        echo 'list-view' . Tools::safe_output($view);
        ?>">
        ...
        echo Tools::safe_output(lang_Dimension);
        ?></div>
        ...
        echo Tools::safe_output(lang_Operations);
        ?></div>
    </div>

	<input type="hidden" id="file_number" value="<?php 
        echo Tools::safe_output($n_files);
        ?>"/>
	...
	<?php 
		...
		<li data-name="<?php 
            ...
            echo $get_params . rawurlencode($src) . '&' . uniqid();
            ?>">
					<div class="img-precontainer">
						<div class="img-container directory"><span></span>
							<img class="directory-img" src="img/<?php 
            echo Tools::safe_output(FILE_MANAGER_ICON_THEME);
            ?>/folder<?php 
            if ($file == '..') {
                echo '_back';
            }
            ...
            if ($file == '..') {
                ?>
					<div class="box no-effect">
						<h4><?php 
                echo Tools::safe_output(lang_Back);
                ?></h4>
					</div>
				</a>
                ...
                echo Tools::safe_output($file);
                ?>" href="dialog.php?<?php 
                echo Tools::safe_output($get_params . rawurlencode($src) . '&' . uniqid());
                ?>"><?php 
                echo Tools::safe_output($file);
                ?></a>
						</h4>
                ...
                echo Tools::safe_output($file_array['date']);
                ?>"/>
					<input type="hidden" class="size" value="<?php 
                echo Tools::safe_output($file_array['size']);
                ?>"/>
                ...
                echo make_size($file_array['size']);
                ?></div>
					<div class='file-extension'><?php 
                echo lang_Type_dir;
                ?></div>
					<figcaption>
						<a href="javascript:void('')" class="tip-left edit-button rename-folder" title="<?php 
                echo lang_Rename;
                ?>" data-path="<?php 
                echo Tools::safe_output($subdir . $file);
                ?>" data-thumb="<?php 
                echo Tools::safe_output($subdir . $file);
                ?>">
							<i class="icon-pencil"></i>
                        </a>
						<a href="javascript:void('')" class="tip-left erase-button delete-folder" title="<?php 
                echo lang_Erase;
                ?>" data-confirm="<?php 
	...
	<?php 
        ...
        foreach ($files as $nu => $file_array) {
            $file = $file_array['file'];
            if ($file == '.' || $file == '..' || is_dir(FILE_MANAGER_BASE_DIR . $subdir . $file) || !in_array(fix_strtolower($file_array['extension']), get_file_extensions()) || $filter != '' && strpos($file, $filter) === false) {
                continue;
            }
            $file_path = FILE_MANAGER_BASE_DIR . $subdir . $file;
            ...
            if (in_array($extension_lower, get_file_extensions('image'))) {
            ...
            $class_ext = 0;
            if (in_array($extension_lower, get_file_extensions('video'))) {
                $class_ext = 4;
                $is_video = true;
            } elseif (in_array($extension_lower, get_file_extensions('image'))) {
                $class_ext = 2;
            } elseif (in_array($extension_lower, get_file_extensions('audio'))) {
                $class_ext = 5;
                $is_audio = true;
            } elseif (in_array($extension_lower, get_file_extensions('misc'))) {
                $class_ext = 3;
            } else {
                $class_ext = 1;
            }
            if ($src_thumb) {
                if (($src_thumb = preg_replace('#(' . addslashes(FILE_MANAGER_BASE_DIR) . ')#ism', Tools::safe_output(Context::get_context()->shop->physical_uri . 'img/cms/'), $src_thumb)) == $src_thumb) {
                    $src_thumb = preg_replace('#(' . addslashes(FILE_MANAGER_THUMB_BASE_DIR) . ')#ism', Tools::safe_output(Context::get_context()->shop->physical_uri . 'img/tmp/cms/'), $src_thumb);
                }
            }
            if ($mini_src) {
                if (($mini_src = preg_replace('#(' . addslashes(FILE_MANAGER_BASE_DIR) . ')#ism', Tools::safe_output(Context::get_context()->shop->physical_uri . 'img/cms/'), $mini_src)) == $mini_src) {
                    $mini_src = preg_replace('#(' . addslashes(FILE_MANAGER_THUMB_BASE_DIR) . ')#ism', Tools::safe_output(Context::get_context()->shop->physical_uri . 'img/tmp/cms/'), $mini_src);
                }
            }
            if (!(Tools::get_value('type') == 1 && !$is_img) && !(Tools::get_value('type') == 3 && !$is_video && (Tools::get_value('type') == 3 && !$is_audio))) {
                ?>
	<li class="ff-item-type-<?php 
                echo Tools::safe_output($class_ext);
                ?> file" data-name="<?php 
                echo Tools::safe_output($file);
                ?>">
		<figure data-name="<?php 
			...
			<a href="javascript:void('')" class="link" data-file="<?php 
                echo Tools::safe_output($file);
                ?>" data-field_id="" data-function="<?php 
                echo Tools::safe_output($apply);
                ?>">
				<div class="img-precontainer">
					<?php 
                if ($is_icon_thumb) {
                ...
                ?>
					<div class="img-container">
						<span></span>
						<img alt="<?php 
                echo Tools::safe_output($filename . ' thumbnails');
                ?>" class="<?php 
                echo $show_original ? 'original' : '';
                ?> <?php 
                echo $is_icon_thumb ? 'icon' : '';
                ?>" src="<?php 
                echo Tools::safe_output($src_thumb);
                ?>">
					</div>
				</div>
                ...
                if ($mini_src != '') {
                    ...
                    ?> " src="<?php 
                    echo Tools::safe_output($mini_src);
                    ?>">
						<?php 
                }
                ?>
			...
			<div class="box">
                ...
                ?>">
						<?php 
                echo Tools::safe_output($filename);
                ?>
                    </a>
                </h4>
			</div>
			...
			<figcaption>
                ...
                ?>"/>
					...
					<a title="<?php 
                echo lang_Download;
                ?>" class="tip-right" href="javascript:void('')" onclick="$('#form<?php 
                echo Tools::safe_output($nu);
                ?>').submit();">
                ...
                if ($is_img && $src_thumb != '') {
                    ...
                    echo Tools::safe_output($filename);
                    ?>&file=<?php 
                    echo Tools::safe_output(Context::get_context()->shop->physical_uri . 'img/cms/' . $subdir . $file);
                    ?>"
                ...
                ?>
					<a href="javascript:void('')" class="tip-left edit-button rename-file" title="<?php 
                echo lang_Rename;
                ?>" data-path="<?php 
                echo Tools::safe_output($subdir . $file);
                ?>" data-thumb="<?php 
                ...
                ?>">
					...
					<a href="javascript:void('')" class="tip-left erase-button delete-file" title="<?php 
                echo lang_Erase;
                ?>" data-confirm="<?php 
